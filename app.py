...
# Tab definitions — these render as a horizontal bar right under the title
tab_home, tab_categories, tab_checklists, tab_fee = st.tabs(
    ["🏠 Home", "📂 Categories", "✅ Checklists", "💰 Fee Calculator"]
)

# ---------------------------------------------------------------------------
# IMPORTANT: st.chat_input must be called at the TOP LEVEL of the script
# (not nested inside `with tab_home:`) for Streamlit to pin it to the
# bottom of the page. Nesting it inside a tab/column/expander disables
# that auto-docking behaviour, which is exactly why it was rendering
# inline near the top before. Capture the value here, then use it below.
# ---------------------------------------------------------------------------
if "messages" not in st.session_state:
    st.session_state.messages = []

question = st.chat_input("Ask about BIS certification, standards, fees…")
...
with tab_home:
    ...
    # Sidebar — sample questions + clear chat
    with st.sidebar:
